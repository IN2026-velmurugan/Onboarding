...
class NIDCPowerSourceDCVoltageClient:
    ...
    def _create_measure_request(
        ...
        serialized_configuration = any_pb2.Any(
            type_url="type.googleapis.com/ni.examples.NIDCPowerSourceDCVoltage_Python.Configurations",
    ...
    def _validate_response(self, response: v2_measurement_service_pb2.MeasureResponse) -> None:
        expected_type = (
            "type.googleapis.com/" + "ni.examples.NIDCPowerSourceDCVoltage_Python.Outputs"
        )
...
if __name__ == "__main__":
    # Example usage of the NIDCPowerSourceDCVoltageClient
    client = NIDCPowerSourceDCVoltageClient()
    client.register_pin_map(
        pathlib.Path(
            "C:/Users/velmurugan.kl/Desktop/measurement-plugin-python/examples/nidcpower_source_dc_voltage/NIDCPowerSourceDCVoltage.pinmap"
        )
    )
    ...
    outputs = client.measure(
        ...
        current_limit=0.02,
        current_limit_range=0.05,
        source_delay=0.1,
    )

    print("Measurement Sites:", outputs.measurement_sites)
    print("Measurement Pin Names:", outputs.measurement_pin_names)
    print("Voltage Measurements:", outputs.voltage_measurements)
    print("Current Measurements:", outputs.current_measurements)
    print("In Compliance:", outputs.in_compliance)
